...
Toolbar = function (dgrm, params) {
    ...
    toolbar.defaultNodePalette = [{
        ...
        'icon': dgrm.imagesPath + 'node.gif',
        'nodeType': 'NODE',
        'nodeContent': '<a href="javascript:;" class="ablock">myID</a><b>Name</b><input type="text"/><input type="submit"/>',
        'width': '100',
        'height': '100',
        'bgColor': '#FFFFFF',
        'borderColor': '#AAAAAA',
    ...
    if (toolbarExists) {
        ...
        if (toolbar.save_button) {
            $("#save_button_" + dgrm.canvasId).button({
                icons: {
                    primary: "save_diagram"
                }
            }).click(function () {
                dgrm.toXML()
            })
...
Diagram = function (params) {
    ...
    this.tempLine = null;
    var diagramcontainer = '<div style="position:absolute; left:' + self.xPosition + 'px;top:' + (self.yPosition + 40) + 'px;width:' + (self.width + 4) + 'px;height:' + (self.height + 4) + 'px;z-index:10018;padding:0px; margin:0px;" class="ui-widget-header ui-corner-all"></div>';
    diagramcontainer += '<div id="' + self.canvid + '" style="position:absolute; left:' + (self.xPosition + 2) + 'px;top:' + (self.yPosition + 42) + 'px;width:' + (self.width) + 'px;height:' + (self.height) + 'px;overflow:scroll;border:1px solid #AAAAAA; background: url(\'' + self.imagesPath + 'bg.gif\') repeat;z-index:10020;"><div id="' + self.canvid + '_templine" style="position: absolute;z-index:10020;"></div></div>';
    ...
    this.addNode = function (node) {
        ...
        else container += unescape(node.nodeContent);
    ...
    this.addConnection = function (connection) {
        ...
        $("#" + connectionId).click(function () {
            caller.deleteConnection(connectionId)
        });
    ...
    this.redrawNodeAffectedConnections = function (nodeId) {
        ...
        for (i = 0; i < affectedConnections.length; i++) {
            dc(affectedConnections[i].nodeFrom, affectedConnections[i].nodeTo, affectedConnections[i].portFrom, affectedConnections[i].portTo, affectedConnections[i].line, caller)
        }
    };
    ...
    this.getSelectedNodes = function () {
        var selections = [];
        for (i = 0; i < this.nodes.length; i++) if (this.nodes[i].isSelected) selections.push(this.nodes[i]);
        return selections
    };
